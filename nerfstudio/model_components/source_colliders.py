...
class SourceCollider(nn.Module):
    ...
    def _intersect_with_plane(self, rays_o: torch.Tensor, rays_d: torch.Tensor):
        ...
        x_intersect = new_rays_o[:,0] - new_rays_o[:,2]*new_rays_d[:,0]/new_rays_d[:,2]
        y_intersect = new_rays_o[:,1] - new_rays_o[:,2]*new_rays_d[:,1]/new_rays_d[:,2]
        return x_intersect, y_intersect

    def _is_touching_source(self, plane_intersection: torch.Tensor):
        """To be implemented."""
        raise NotImplementedError
    
    def set_source_intersection(self, ray_bundle: RayBundle) -> RayBundle:
        plane_x_intersection, plane_y_intersection = self._intersect_with_plane(rays_o=ray_bundle.origins, rays_d=ray_bundle.directions)
        ...
        source_intersection = self._is_touching_source(plane_intersection)
...
class RectangleSourceCollider(SourceCollider):
    ...
    def _is_touching_source(self, plane_intersection: torch.Tensor):
         plane_intersection[(torch.abs(plane_intersection[:,0]) > self.X_size/2) | (torch.abs(plane_intersection[:,1]) > self.Y_size/2)] = torch.nan
...
class EllipseSourceCollider(SourceCollider):
    ...
    def _is_touching_source(self, plane_intersection: torch.Tensor):
         plane_intersection[plane_intersection[:,0]**2 / (self.X_size/2)**2 + plane_intersection[:,1]**2 / (self.Y_size/2)**2 > 1] = torch.nan
         return plane_intersection
